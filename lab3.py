...
def check_in_cache(adres,size):
    ...
    for i in range(4):
        #print(i,idx,CACHE_SETS_COUNT,CACHE_WAY)
        if(cache[idx][i][0]==tag):
            #LRU_tacts+=6+1+ int(size==4)
            max1 = 0
            for j in range(4):
            ...
            cache_in+=1
            cache[idx][i][1]=1
            LRU_tacts += 7 + size//ADDR1_BUS_LEN
            break
    else:
        cache_out+=1
        LRU_tacts+=4
        ...
        #LRU_tacts += 1 + size//ADDR1_BUS_LEN
    ...
    for y in range(64):
        ...
        for x in range(60):
            ...
            for k in range(32):
                #LRU_tacts+=2
                s+=a[y][k]*b[k][x]
                check_in_cache(bin(int('400000',16)+y * K + k)[2:], 16)
                #LRU_tacts+=2
                check_in_cache(bin(K*M+int('400000',16)+k*N*2+x*2)[2:], 32)
                LRU_tacts+=7
                LRU_tacts+=2
            c[y][x] = s
            check_in_cache(bin(x*4+N*y*4+int('400000',16)+K*M+N*K*2)[2:], 64)
        LRU_tacts+=2
nmul()
LRU_tacts+=1
...
def check_in_cache_plru(adres,size):
    ...
    for i in range(4):
        if (cache[idx][i][0] == tag):
            PLRU_tacts += 6 + 1 + size//ADDR1_BUS_LEN
            cache[idx][i][2] = 1
            cache_in_plru += 1
            count_zer = 0
            for j in range(4):
        ...
        #PLRU_tacts += 1 + size//ADDR1_BUS_LEN
        ind  = 0
        count_zer = 0
        for j  in range(4):
            if cache[idx][j][2]==0:
    ...
    for y in range(64):
        ...
        for x in range(60):
            ...
            for k in range(32):
                PLRU_tacts+=2
                s+=a[y][k]*b[k][x]
                check_in_cache_plru(bin(int('400000',16)+y * K + k)[2:], 16)
                #PLRU_tacts+=2
                check_in_cache_plru(bin(K*M+int('400000',16)+k*N*2+x*2)[2:], 32)
                PLRU_tacts+=7
                #PLRU_tacts+=1
            c[y][x] = s
            check_in_cache_plru(bin(x*4+N*y*4+int('400000',16)+K*M+N*K*2)[2:], 64)
        PLRU_tacts+=2
nmul()
PLRU_tacts+=1
index = 0
def check_in_cache_rr(adres,size):
    global cache, cache_in_rr, RR_tacts, cache_out_rr,index,indexes
    ofset = int(adres[-CACHE_OFFSET_LEN:], 2)
    idx = int(adres[-(CACHE_IDX_LEN + CACHE_OFFSET_LEN):-CACHE_OFFSET_LEN], 2)
    tag = int(adres[:-(CACHE_IDX_LEN + CACHE_OFFSET_LEN)], 2)
    for i in range(4):
        ...
        #RR_tacts += 1 + size // ADDR_LEN
        ind = 0
        if cache[idx][index][1]==1:
            RR_tacts+=17
        cache[idx][index][0] = tag
        cache[idx][index][1] = 0
        cache[idx][index][2]=0
        index+=1
        index%=4
...
def nmul():
    global RR_tacts, RR_tacts, a, b, c, M, N, K
    RR_tacts += 1
    RR_tacts += 2
    for y in range(64):
        RR_tacts += 3
        for x in range(60):
            ...
            for k in range(32):
                RR_tacts += 2
                s += a[y][k] * b[k][x]
                check_in_cache_rr(bin(int('400000', 16) + y * K + k)[2:], 16)
                #RR_tacts += 2
                check_in_cache_rr(bin(K * M + int('400000', 16) + k * N * 2 + x * 2)[2:], 32)
                RR_tacts += 7
                #RR_tacts += 1
            c[y][x] = s
            check_in_cache_rr(bin(x * 4 + N * y * 4 + int('400000', 16) + K * M + N * K * 2)[2:], 64)
        RR_tacts += 2
nmul()
RR_tacts+=1
#print("LRU:\thit perc. {:.4f}%\ttime: {time1}\npLRU:\thit perc. {:.4f}%\ttime: {time2}\n".format(cache_in / (cache_out+cache_in) * 100, cache_in_plru / (cache_out_plru+cache_in_plru) * 100, time1 = LRU_tacts, time2 = PLRU_tacts))
...
#print(cache_in_rr / (cache_out_rr+cache_in_rr) * 100,RR_tacts)
